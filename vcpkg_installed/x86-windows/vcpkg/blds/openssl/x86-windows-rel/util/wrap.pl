...
BEGIN {
    ...
    require 'C:/repos/RAT/vcpkg_installed/x86-windows/vcpkg/blds/openssl/x86-windows-rel/util/perl/OpenSSL/Util.pm';
    OpenSSL::Util->import();
}
...
if ($ARGV[0] eq '-fips') {
    $std_openssl_conf = 'C:/repos/RAT/vcpkg_installed/x86-windows/vcpkg/blds/openssl/x86-windows-rel/test/fips-and-base.cnf';
    shift;
...
if ($ARGV[0] eq '-jitter') {
    $std_openssl_conf = 'C:/repos/RAT/vcpkg_installed/x86-windows/vcpkg/blds/openssl/x86-windows-rel/test/default-and-jitter.cnf';
    shift;
